...
# heavily borrowed from https://github.com/khesui/FPMC
...
class FPMC():
    def __init__(self, n_user, n_loc, n_factor, learn_rate, regular):
        self.user_set = set()
        self.loc_set = set()
    ...
    def init_model(self, std=0.01):
        ...
        self.VIU = np.random.normal(0, std, size=(self.n_loc, self.n_factor))
        self.VIL = np.random.normal(0, std, size=(self.n_loc, self.n_factor))
        self.VLI = np.random.normal(0, std, size=(self.n_loc, self.n_factor))
    ...
    def evaluation(self, data_list):
        ...
        for (u, i, b_tm1) in data_list:
            scores = self.compute_x_batch(u, b_tm1) # 相当于预测一次
